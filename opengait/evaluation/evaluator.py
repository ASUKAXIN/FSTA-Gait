...
def GREW_submission(data, dataset, metric='euc'):
    """
    评估 GREW 数据集上的模型表现，并将结果保存为 CSV 文件。

    参数:
    data (dict): 包含嵌入向量、标签、序列类型和视角信息的字典。
    dataset (str): 数据集名称，这里应该是 'GREW'。
    metric (str, 可选): 计算距离的度量方法，默认为 'euc'（欧几里得距离）。
    """
    get_msg_mgr().log_info("Evaluating GREW")
    # 从输入数据字典中提取嵌入向量、标签、序列类型和视角信息
    feature, label, seq_type, view = data['embeddings'], data['labels'], data['types'], data['views']
    # 将标签转换为 NumPy 数组
    label = np.array(label)
    # 将视角转换为 NumPy 数组
    ...
    # 定义画廊序列类型，对于 GREW 数据集，画廊序列类型为 '01' 和 '02'
    gallery_seq_type = {'GREW': ['01', '02']}
    # 定义探针序列类型，对于 GREW 数据集，探针序列类型为 '03'
    probe_seq_type = {'GREW': ['03']}

    # 创建一个布尔掩码，标记哪些序列属于画廊序列类型
    gseq_mask = np.isin(seq_type, gallery_seq_type[dataset])
    # 从特征矩阵中提取画廊样本的特征
    gallery_x = feature[gseq_mask, :]
    # 从标签数组中提取画廊样本的标签
    ...
    # 设置排名的数量为 20
    num_rank = 20
    # 使用指定的度量方法计算探针样本和画廊样本之间的距离
    dist = cuda_dist(probe_x, gallery_x, metric)
    # 获取距离矩阵中每个探针样本最近的 num_rank 个画廊样本的索引
    idx = dist.topk(num_rank, largest=False)[1].cpu().numpy()

    # 构建保存结果的 CSV 文件路径，文件名为当前时间戳
    save_path = os.path.join(
        "GREW_result/"+strftime('%Y-%m%d-%H%M%S', localtime())+".csv")
    # 创建 GREW_result 目录（如果不存在）
    ...
    with open(save_path, "w") as f:
        ...
        for i in range(len(idx)):
            # 将每个探针样本对应的最近的 num_rank 个画廊样本的标签转换为整数列表
            r_format = [int(idx) for idx in gallery_y[idx[i, 0:num_rank]]]
            # 定义输出行的格式
            output_row = '{}'+',{}'*num_rank+'\n'
            # 将探针样本的视角和最近的画廊样本的标签写入 CSV 文件
